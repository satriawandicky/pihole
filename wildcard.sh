...
curl -sS https://raw.githubusercontent.com/satriawandicky/pihole/master/wildcard.txt | sudo tee -a "${PIHOLE_LOCATION}"/wildcard.txt && sort "${PIHOLE_LOCATION}"/wildcard.txt | uniq > /dev/null
